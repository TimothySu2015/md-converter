...
const { getMermaidScriptTag } = require('./mermaidResources');
...
async function convertMarkdownToHtml(markdownPathOrContent, title = 'Document') {
  ...
  // In marked v15+, heading renderer receives a token object
  ...
  // 取得 Mermaid script tag（優先使用本地，失敗則使用 CDN）
...
async function convertHtmlToPdf(htmlContent, outputPath, options = {}) {
  ...
  for (const match of matches) {
    ...
    // Skip if already absolute URL, data URL, or invalid template string
  ...
  await page.setContent(htmlContent, {
    ...
    timeout: 60000 // 增加到 60 秒以應對 CDN 載入慢的情況
  });
  ...
  await page.waitForFunction(
    ...
    { timeout: 30000 } // 增加等待 Mermaid 載入的時間
  ).catch(() => {
    console.log('⚠ Warning: Mermaid library not loaded within 30 seconds');
  });
