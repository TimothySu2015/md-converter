...
class CompleteMarkdownToDocxConverter {
   ...
   * @param {number} maxRedirects - 最大重定向次數（預設 5）
   * @returns {Promise<Buffer>} 圖片 Buffer
   */
  async downloadImage(url, maxRedirects = 5) {
    // 檔案大小上限：50MB
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = protocol.get(options, (response) => {
        ...
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
          ...
          this.downloadImage(redirectUrl, maxRedirects - 1).then(resolve).catch(reject);
          return;
        }
        ...
        response.on('data', (chunk) => {
          ...
          if (totalSize > MAX_IMAGE_SIZE) {
            request.destroy();
            reject(new Error(`圖片檔案過大（超過 ${MAX_IMAGE_SIZE / 1024 / 1024}MB）`));
            return;
          }
          chunks.push(chunk);
        });
   ...
   * @param {string|Buffer} svgPathOrContent - SVG 檔案路徑、SVG 內容字串或 Buffer
   */
  async convertSvgToPng(svgPathOrContent) {
    let page = null;
    try {
      if (!this.browser) {
      ...
      // 支援檔案路徑、字串內容或 Buffer
      let svgContent;
      if (Buffer.isBuffer(svgPathOrContent)) {
        svgContent = svgPathOrContent.toString('utf8');
      } else if (fs.existsSync(svgPathOrContent)) {
        svgContent = fs.readFileSync(svgPathOrContent, 'utf8');
      } else {
        // 假設是 SVG 內容字串
        svgContent = svgPathOrContent;
      }
  ...
  async addImage(token, children) {
    try {
      ...
      if (isUrl) {
        ...
        console.log(`  下載網路圖片...`);
        try {
          imageBuffer = await this.downloadImage(imagePath);
          // 從 URL 或 Content-Type 推斷副檔名
          const urlPath = new URL(imagePath).pathname;
          ext = path.extname(urlPath).toLowerCase() || '.png';
          console.log(`  下載完成 (${(imageBuffer.length / 1024).toFixed(1)} KB)`);
        } catch (downloadError) {
          ...
          children.push(
            new Paragraph({
              children: [
                new TextRun({
                  text: `[圖片下載失敗: ${token.text || imagePath}]`,
                  italics: true,
        ...
        if (!fs.existsSync(imagePath)) {
          ...
          children.push(
            new Paragraph({
              children: [
                new TextRun({
                  text: `[圖片不存在: ${token.text || token.href}]`,
                  italics: true,
                  size: pStyle.text?.fontSize || 24,
                  font: this.options.fontFamily
                })
      ...
      if (imageBuffer) {
        ...
        if (ext === '.svg') {
          const svgName = isUrl ? token.text || 'SVG' : path.basename(imagePath);
          console.log(`  轉換 SVG 為 PNG: ${svgName}`);
          try {
            // 對於網址圖片使用 Buffer，對於本地檔案使用路徑
            imageBuffer = await this.convertSvgToPng(isUrl ? imageBuffer : imagePath);
          } catch (svgError) {
            ...
            children.push(
              new Paragraph({
                children: [
                  new TextRun({
                    text: `[SVG 圖片: ${token.text || svgName}]`,
                    italics: true,
                    size: 22,
                    font: this.options.fontFamily
                  })
