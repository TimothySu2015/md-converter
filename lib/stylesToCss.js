...
/**
 * 取得字體 CSS（優先使用 fontsource 本地字體，失敗則使用 Google Fonts CDN）
 * 使用 file:// 協議讓 Puppeteer 直接讀取本地檔案，避免 base64 編碼造成的效能問題
 */
let _fontCssCache = undefined;
function getFontCss() {
  // 使用快取避免重複讀取
  if (_fontCssCache !== undefined) {
    return _fontCssCache;
  }

  // 定義要載入的字體 CSS（使用 fontsource）
  const fontPackages = [
    { name: '@fontsource/noto-sans-tc', weights: ['300', '400', '500', '700'] },
    { name: '@fontsource/noto-serif-tc', weights: ['400', '700'] }
  ];
  ...
  for (const pkg of fontPackages) {
    ...
    let pkgDir;
    try {
      pkgDir = path.dirname(require.resolve(`${pkg.name}/package.json`));
    } catch (e) {
      continue; // 套件未安裝，跳過
    }

    for (const weight of pkg.weights) {
      const cssFile = path.join(pkgDir, `${weight}.css`);
      if (!fs.existsSync(cssFile)) continue;

      let fontCss = fs.readFileSync(cssFile, 'utf8');
      const filesDir = path.join(pkgDir, 'files');

      // 將相對路徑 ./files/xxx 轉換為 file:// 絕對路徑
      // 處理 Windows 路徑：反斜線轉正斜線，空格編碼為 %20
      const filesDirUrl = filesDir.replace(/\\/g, '/').replace(/ /g, '%20');
      fontCss = fontCss.replace(/url\(\.\/files\//g, `url(file:///${filesDirUrl}/`);
